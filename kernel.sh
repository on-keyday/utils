...
STD_STUB_SRC="$STD_DIR/stub/heap_stub.cpp"
STUB_HEAP_DEFINE="-D FUTILS_FREESTD_HEAP_MEMORY_SIZE=0x100000"
...
$CC $CFLAGS -Wl,-T$SHELL_LINKER_SCRIPT -Wl,-Map=$SHELL_MAP $STD_INCLUDE_DIR $FREESTD_DEFINE -o $SHELL_ELF $SHELL_SRC  $STD_SRC $STD_STUB_SRC $STUB_HEAP_DEFINE
if [ $? -ne 0 ]; then
    echo "Build failed."
    return 1
fi
...
# build kernel
$CC $CFLAGS -Wl,-T$KERNEL_LINKER_SCRIPT -Wl,-Map=$KERNEL_MAP -o $OUTPUT $STD_INCLUDE_DIR $FREESTD_DEFINE $STD_SRC $KERNEL_C_SRC $KERNEL_CPP_SRC $SHELL_BIN_O $STD_STUB_SRC
