...
#include <string>
#include <random>
#include <dnet/quic/old/frame_old/frame.h>
#include <dnet/quic/old/frame_old/frame_util.h>
#include <dnet/quic/old/packet_old/packet.h>
#include <dnet/socket.h>
#include <dnet/plthead.h>
#include <dnet/addrinfo.h>
#include <dnet/quic/old/frame_old/vframe.h>
#include <dnet/quic/old/packet_old/packet_util.h>
#include <dnet/quic/old/frame_old/frame_make.h>
...
void test_dnetquic_initial() {
   ...
   res = dnet::quic::crypto::make_initial_secret(view::wvec(secret.data, secret.len), 1, view::rvec(packet.first.dstID.data, packet.first.dstID.len), true);
   assert(res);
   dnet::quic::crypto::Keys keys;
   res = dnet::quic::crypto::make_keys_from_secret(keys, {}, 1, view::rvec(secret.data, secret.len)).is_noerr();
   assert(res);
   auto comp = info.composition();
   dnet::quic::packet::CryptoPacket parsed;
   parsed.src = view::wvec(comp.data, comp.len);
   parsed.head_len = info.head.len;
   res = dnet::quic::crypto::encrypt_packet(keys, {}, parsed).is_noerr();
