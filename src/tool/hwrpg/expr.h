...
namespace expr {
    ...
    struct UnaryExpr : Expr {
        ...
        EvalState eval(RuntimeState& s) const override {
            if (s.stacks.eval_stack.empty()) {
                return s.eval_error(U"スタックが空です(単項演算子)");
            }
            auto v_ = s.stacks.eval_stack.back().eval(s);
            ...
            switch (op) {
                case UnaryOp::plus:
                    ...
                    if (auto b = v.boolean()) {
                        s.stacks.eval_stack.push_back(EvalValue(*b));
                        return EvalState::normal;
                    }
                ...
                case UnaryOp::bit_not:
                    if (auto n = v.number()) {
                        s.stacks.eval_stack.push_back(EvalValue(std::int64_t(~std::uint64_t(*n))));
                        return EvalState::normal;
                    }
                    break;
            }
        ...
        void push(RuntimeState& s) override {
            s.stacks.expr_stack.push_back(shared_from_this());
            value->push(s);
        }
    };
    ...
    struct BinaryExpr : Expr {
        ...
        EvalState eval_number(RuntimeState& s, std::int64_t left, std::int64_t right) const {
            switch (op) {
                case BinaryOp::add:
                    s.stacks.eval_stack.push_back(EvalValue(left + right));
                    return EvalState::normal;
                case BinaryOp::sub:
                    s.stacks.eval_stack.push_back(EvalValue(left - right));
                    return EvalState::normal;
                ...
                case BinaryOp::mod:
                    ...
                    return EvalState::normal;
                case BinaryOp::eq:
                    s.stacks.eval_stack.push_back(EvalValue(Boolean{left == right}));
                    return EvalState::normal;
                case BinaryOp::ne:
                    s.stacks.eval_stack.push_back(EvalValue(Boolean{left != right}));
                    return EvalState::normal;
                ...
                case BinaryOp::bit_and:
                    s.stacks.eval_stack.push_back(EvalValue(std::int64_t(std::uint64_t(left) & std::uint64_t(right))));
                    return EvalState::normal;
                ...
                case BinaryOp::bit_right_arithmetic_shift:
                    s.stacks.eval_stack.push_back(EvalValue(std::int64_t(std::int64_t(left) >> std::uint64_t(right))));
                    return EvalState::normal;
        ...
        EvalState eval(RuntimeState& s) const override {
            if (op == BinaryOp::and_ || op == BinaryOp::or_) {
                if (s.stacks.eval_stack.size() < 1) {
                    return s.eval_error(U"スタックが空です(短絡評価)");
                }
                auto sh_ = s.stacks.eval_stack.back().eval(s);
                s.stacks.eval_stack.pop_back();
                if (!sh_) {
                    return EvalState::error;  // already reported
                }
                ...
                if (remove_short_circuit) {
                    if (s.stacks.expr_stack.size() < short_circuit) {
                        return s.eval_error(U"スタックが空です(短絡評価)");
                    }
                    for (size_t i = 0; i < short_circuit; i++) {
                        s.stacks.expr_stack.pop_back();
                    }
                    s.stacks.eval_stack.push_back(EvalValue(Boolean{sh->value}));
                }
                return EvalState::normal;
            }
            if (s.stacks.eval_stack.size() < 2) {
                return s.eval_error(U"スタックが空です(二項演算子)");
            }
            auto left_ = s.stacks.eval_stack.back().eval(s);
            s.stacks.eval_stack.pop_back();
            if (!left_) {
                return EvalState::error;  // already reported
            }
            auto right_ = s.stacks.eval_stack.back().eval(s);
            s.stacks.eval_stack.pop_back();
            if (!right_) {
                return EvalState::error;  // already reported
            }
            ...
            if (left_null && right_null) {
                switch (op) {
                    case BinaryOp::eq:
                        s.stacks.eval_stack.push_back(EvalValue(Boolean{true}));
                        return EvalState::normal;
                    case BinaryOp::ne:
                        s.stacks.eval_stack.push_back(EvalValue(Boolean{false}));
                        return EvalState::normal;
                    default:
        ...
        void push(RuntimeState& s) override {
            if (op == BinaryOp::and_ || op == BinaryOp::or_) {
                size_t start = s.stacks.expr_stack.size();
                right->push(s);
                size_t end = s.stacks.expr_stack.size();
    ...
    struct IdentifierExpr : Expr {
        std::u32string name;
        EvalState eval(RuntimeState& s) const override {
            s.stacks.eval_stack.push_back(Assignable{name});
            return EvalState::normal;
        }
    ...
    struct IntLiteralExpr : Expr {
        std::int64_t value = 0;
        EvalState eval(RuntimeState& s) const override {
            s.stacks.eval_stack.push_back(EvalValue(value));
            return EvalState::normal;
        }
    ...
    struct ArrayLiteralExpr : Expr {
        ...
        EvalState eval(RuntimeState& s) const override {
            if (s.stacks.eval_stack.size() < elements.size()) {
                return s.eval_error(U"スタックが空です(配列リテラル)");
            }
            std::vector<Value> arr;
            for (auto& e : elements) {
                auto v = s.stacks.eval_stack.back().eval(s);
                s.stacks.eval_stack.pop_back();
                if (!v) {
                    return EvalState::error;  // already reported
                }
    ...
    struct ObjectLiteralExpr : Expr {
        ...
        EvalState eval(RuntimeState& s) const override {
            ...
            s.stacks.eval_stack.push_back(EvalValue(obj));
            return EvalState::normal;
        }
    ...
    struct SpecialObjectExpr : Expr {
        ...
        EvalState eval(RuntimeState& s) const override {
            ...
            s.stacks.eval_stack.push_back(EvalValue(access));
            return EvalState::normal;
        }
    ...
    struct BoolLiteralExpr : Expr {
        bool value;
        EvalState eval(RuntimeState& s) const override {
            s.stacks.eval_stack.push_back(EvalValue(Boolean{value}));
            return EvalState::normal;
        }
    ...
    struct DotAccessExpr : Expr {
        ...
        EvalState eval(RuntimeState& s) const override {
            ...
            auto v = s.stacks.eval_stack.back();
            s.stacks.eval_stack.pop_back();
            if (auto access = v.access()) {
                ...
                s.stacks.eval_stack.push_back(EvalValue(std::int64_t(a->size())));
            }
            else {
    ...
    struct IndexAccessExpr : Expr {
        ...
        EvalState eval(RuntimeState& s) const override {
            ...
            auto v = s.stacks.eval_stack.back();
            s.stacks.eval_stack.pop_back();
            auto index = s.stacks.eval_stack.back().eval(s);
            s.stacks.eval_stack.pop_back();
            if (!index) {
                return EvalState::error;  // already reported
            }
    ...
    struct AssignExpr : Expr {
        ...
        EvalState eval(RuntimeState& s) const override {
            ...
            auto target = s.stacks.eval_stack.back();
            s.stacks.eval_stack.pop_back();
            auto value_ = s.stacks.eval_stack.back().eval(s);
            s.stacks.eval_stack.pop_back();
            if (!value_) {
                return EvalState::error;  // already reported
            }
            ...
            s.stacks.eval_stack.push_back(target);
            return EvalState::normal;
        }
    ...
    struct CallExpr : Expr {
        ...
        EvalState eval(RuntimeState& s) const override {
            if (s.stacks.eval_stack.size() < args.size() + 1) {
                return s.eval_error(U"スタックが空です(関数呼び出し)");
            }
            auto func_name = s.stacks.eval_stack.back();
            s.stacks.eval_stack.pop_back();
