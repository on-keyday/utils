...
void pkt(std::shared_ptr<void>& ctx, futils::view::rvec traceID, futils::fnet::quic::path::PathID path, futils::fnet::quic::packet::PacketSummary summary, futils::view::rvec payload, bool is_send) {
    auto ptr = std::static_pointer_cast<ServerContext>(ctx);
    auto addr = ptr->hm->get_peer_addr(path);
...
futils::fnet::quic::log::ConnLogCallbacks log_cb{
    .debug = [](std::shared_ptr<void>&, futils::view::rvec traceID, const char* str) { cout << futils::wrap::pack(futils::number::hex::to_hex<std::string>(traceID), " ", str, "\n"); },
    .report_error = [](std::shared_ptr<void>&, futils::view::rvec traceID, const futils::fnet::error::Error& err) { cout << futils::wrap::pack(futils::number::hex::to_hex<std::string>(traceID), " ", err.error(), "\n"); },
    .sending_packet = pkt,
    .recv_packet = pkt,
};
...
int quic_server(Flags& flag) {
    ...
    cout << futils::wrap::pack("server listening on ", flag.port, "\n");
    while (!ctx->end) {
