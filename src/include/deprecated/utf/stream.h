...
namespace utils::utf {
    ...
    struct Stream {
        ...
        UTFErr read_one(auto& out, ConvertMode cvt = ConvertMode::none, SameSizeMode smode = SameSizeMode::none) {
            if (type == UTFType::unknown) {
                size_t skip;
                std::tie(type, skip) = decide_type(seq);
                seq.consume(skip);
            }
            auto do_convert = [&](auto& view, size_t width) -> UTFErr {
                auto seq = make_ref_seq(view);
                if (auto ok = utf::convert_one(seq, out, cvt, smode); !ok) {
                    return ok;
                }
