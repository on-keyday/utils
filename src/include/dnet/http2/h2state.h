    ...
    namespace dnet {
        namespace h2stream {
            ...
            struct StreamNumDirState {
                ...
                h2frame::FrameType preproced_type = h2frame::FrameType::settings;
            ...
            constexpr bool change_state_send(State& state, ErrCode& errs, bool& on_continuous, const h2frame::Frame& f) {
                using t = h2frame::FrameType;
                using l = h2frame::Flag;
                using a = State;
                if (on_continuous) {
                ...
                if (state == a::closed) {
                    if (!changed_now &&
                        ...
                        errs.err = http2_stream_state;
                        errs.h2err = h2frame::H2Error::stream_closed;
            ...
            // - s.id != f.id
            constexpr bool change_state_recv(State& state, ErrCode& errs, bool& on_continuous, h2frame::Frame& f) {
                using t = h2frame::FrameType;
                using l = h2frame::Flag;
                using a = State;
                if (on_continuous) {
            ...
            struct ConnNumDirState {
                ...
                h2frame::FrameType preproced_type = h2frame::FrameType::invalid;
                ...
                h2frame::FrameType preproced_stream_type = h2frame::FrameType::invalid;
                // predefine settings
            ...
            constexpr bool is_stream_level(h2frame::FrameType type) {
                using t = h2frame::FrameType;
            ...
            constexpr bool is_connection_level(h2frame::FrameType type) {
                using t = h2frame::FrameType;
            ...
            constexpr bool has_frame_state_error(
                bool send, h2set::PredefinedSettings& ps, State state, ErrCode& errs, h2frame::Frame& f) {
                using t = h2frame::FrameType;
                using l = h2frame::Flag;
                using a = State;
            ...
            constexpr bool update_window(ConnNumState& c, StreamNumState& s, h2frame::Frame& f, bool on_send) {
                ...
                if (f.type == h2frame::FrameType::data) {
                    if (s.com.id != f.id) {
                ...
                else if (f.type == h2frame::FrameType::window_update) {
                    auto& w = static_cast<h2frame::WindowUpdateFrame&>(f);
                    if (w.id == 0) {
            ...
            constexpr bool check_data_window(std::int64_t cwindow, std::int64_t swindow, ErrCode& errs, h2frame::Frame& f) {
                if (f.type == h2frame::FrameType::data) {
                    if (cwindow - f.len < 0 || swindow - f.len < 0) {
                        errs.err = http2_should_reduce_data;
                        errs.h2err = h2frame::H2Error::enhance_your_clam;
            ...
            constexpr bool check_continous(int preproced_id, ErrCode& errs, h2frame::Frame& f) {
            ...
            constexpr bool recv_frame(ConnNumState& c, StreamNumState& s, h2frame::Frame& f) {
            ...
            constexpr bool send_frame(ConnNumState& c, StreamNumState& s, h2frame::Frame& f) {
                if (c.s0.send.on_continuous) {
                    if (c.send.preproced_id != f.id) {
                        s.com.errs.err = http2_invalid_header_blocks;
                        s.com.errs.h2err = h2frame::H2Error::protocol;
