...
dnet_dll_export(int) parse_h2frame(const char* text, size_t size, size_t* red, H2ErrCode* err, H2ReadCallback cb, void* c);
dnet_dll_export(int) render_h2frame(char* text, size_t* size, size_t cap, H2ErrCode* err, H2Frame* frame, bool auto_correct);
dnet_dll_export(int) pass_h2frame(const char* text, size_t size, size_t* red);
#define INIT(Struct) dnet_dll_export(void) init_##Struct(H2##Struct* ptr);
INIT(Frame)
INIT(DataFrame)
INIT(HeaderFrame)
INIT(PriorityFrame)
