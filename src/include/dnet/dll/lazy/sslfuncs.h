...
#define STACK_OF(Type) STACK_OF_##Type
    ...
    namespace cc {
        namespace ssl {
            ...
            // ex data (on dnet, for QUIC callback)
    ...
    namespace common {
        ...
        int SSL_use_PrivateKey_file(SSL* ctx, const char* file, int type);
        int SSL_check_private_key(const SSL* ctx);
        // TODO(on-keyday): API compatibility is broken between OpenSSL and BoringSSL.
        //                  But ABI (x86-64) level is compatible between these.
        //                  this is may unsafe
        int SSL_set_alpn_protos(SSL* ssl, const unsigned char* protos, unsigned int protos_len);
        void SSL_set_verify(SSL* ctx, int mode,
                            int (*verify_callback)(int, X509_STORE_CTX*));
    ...
    namespace open_ext {
        ...
        unsigned long ERR_get_error_ossl();
        const char* ERR_lib_error_string_ossl(unsigned long e);
        const char* ERR_reason_error_string_ossl(unsigned long e);
    ...
    namespace boring_ext {
        ...
        std::uint32_t ERR_get_error_bssl();
        const char* ERR_lib_error_string_bssl(std::uint32_t e);
        const char* ERR_reason_error_string_bssl(std::uint32_t e);
        const char* SSL_error_description(int err);
    }  // namespace boring_ext
