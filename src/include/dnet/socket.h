...
namespace utils {
    namespace dnet {
        ...
        dnet_dll_export(int) wait_io_event(std::uint32_t time);
        ...
        dnet_dll_export(bool) isSysBlock(const error::Error& err);
        ...
        dnet_dll_export(Socket) make_socket(SockAttr attr);
