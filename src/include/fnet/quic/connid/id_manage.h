...
namespace utils {
    namespace fnet::quic::connid {
        ...
        struct IDManager {
            IDAcceptor acceptor;
            IDIssuer issuer;
            // client: first time generation
            // server: first received
            ...
            view::rvec gen_initial_random(byte len = 8) {
                ...
                if (!issuer.random.gen_random(initial_random)) {
                    return {};
                }
            ...
            error::Error recv(const frame::Frame& f) {
                if (f.type.type_detail() == FrameType::NEW_CONNECTION_ID) {
                    ...
                    recv_retire_connection_id(static_cast<const frame::RetireConnectionIDFrame&>(f));
                }
                return error::none;
            }

            IOResult send_new_connection_id(frame::fwriter& w, auto&& observer_vec) {
            ...
            IOResult send_retire_connection_id(frame::fwriter& w, auto&& observer_vec) {
            ...
            IOResult send(frame::fwriter& w, auto&& observer_vec) {
                if (auto err = send_retire_connection_id(w, observer_vec); err == IOResult::fatal) {
                    return IOResult::fatal;
                }
                return send_new_connection_id(w, observer_vec);
            }
            ...
            bool on_initial_packet_received(view::rvec id) {
            ...
            void on_initial_stateless_reset_token_received(byte (&stateless_reset_token)[16]) {
            ...
            void on_preferred_address_received(view::rvec id, byte (&stateless_reset_token)[16]) {
                acceptor.accept(1, id, stateless_reset_token);
            }

            void on_retry_received(view::rvec id) {
