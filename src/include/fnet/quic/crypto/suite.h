...
namespace utils {
    namespace fnet::quic::crypto {
        ...
        struct CryptoData {
            ...
            using Map = slib::list<CryptoWriteFragment>;
            Map fragments;

            static bool save_new_fragment(auto& frag, packetnum::Value pn, auto&& observer_vec, frame::CryptoFrame frame) {
                CryptoWriteFragment& it = frag.emplace_back(CryptoWriteFragment{});
                it.fragment = frame.crypto_data;
           ...
           private:
            IOResult send_retransmit(packetnum::Value pn, auto&& observer_vec, frame::fwriter& w) {
                ...
                for (auto it = fragments.begin(); it != fragments.end();) {
                    ...
                    if (it->wait->is_lost()) {
                        auto [frame, ok] = frame::make_fit_size_Crypto(w.remain().size(), it->offset, it->fragment.rvec());
                        if (!ok) {
                            it++;
                            continue;
                        }
                        ...
                        if (it->fragment.size() != frame.crypto_data.size()) {
                            size_t new_offset = frame.offset + frame.length;
                            view::rvec remain = it->fragment.rvec().substr(new_offset);
                            it->offset = new_offset;
                            it->fragment = flex_storage(remain);
                        }
                        else {
                            it = fragments.erase(it);
                        }
           ...
           public:
            IOResult send(packetnum::Value pn, auto&& observer_vec, frame::fwriter& w) {
                if (auto res = send_retransmit(pn, observer_vec, w);
                    res == IOResult::fatal) {
                    return res;
                }
