        ...
        namespace internal {
            ...
            struct CookieParser {
                ...
                static void set_by_cookie_prefix(cookie_t& cookie) {
                    if (strutil::starts_with(cookie.name, "__Secure-")) {
                        cookie.secure = true;
                    }
                    else if (strutil::starts_with(cookie.name, "__Host-")) {
                ...
                static CookieErr parse_cookie(const string_t& raw, Cookies& cookies) {
                    auto data = strutil::split<string_t, Vec>(raw, "; ");
                    for (auto& v : data) {
                        auto keyval = strutil::split<string_t, Vec>(v, "=", 1);
                        if (keyval.size() != 2) {
                ...
                static CookieErr parse(const Raw& raw, cookie_t& cookie, const URL& url) {
                    ...
                    auto keyval = strutil::split<string_t, Vec>(data[0], "=", 1);
                    if (keyval.size() != 2) {
                        return CookieError::no_keyvalue;
                    }
                    ...
                    constexpr auto cmp = [](auto& a, auto& b) {
                        return strutil::equal(a, b, strutil::ignore_case());
                    };
                    for (auto& attr : data) {
                        if (cmp(attr, "HttpOnly")) {
                            ...
                            auto elm = strutil::split<string_t, Vec>(attr, "=", 1);
                            if (elm.size() != 2) {
                ...
                static CookieErr parse_set_cookie(Header& header, Cookies& cookies, const URL& url) {
                    for (auto& h : header) {
                        if (strutil::equal(get<0>(h), "Set-Cookie", strutil::ignore_case())) {
                            cookie_t cookie;
               ...
               public:
                ...
                static bool write(string_t& towrite, cookie_t& info, Cookies& cookies) {
                    ...
                    for (size_t i = 0; i < cookies.size(); i++) {
                        ...
                        if (towrite.size()) {
                            strutil::append(towrite, "; ");
                        }
                        strutil::append(towrite, cookie.name);
                        towrite.push_back('=');
                        strutil::append(towrite, cookie.value);
                    }
                ...
                static bool write_set_cookie(string_t& towrite, cookie_t& cookie) {
                    ...
                    strutil::append(towrite, cookie.value);
                    if (cookie.maxage) {
                        strutil::append(towrite, "; MaxAge=");
                        number::to_string(towrite, cookie.maxage);
                    }
                    else if (cookie.expires != date::Date{}) {
                        if (cookie.expires == date::invalid_date) {
                            strutil::append(towrite, "; Expires=-1");
                        }
                        else {
                            strutil::append(towrite, "; Expires=");
                            date::decode(cookie.expires, towrite);
                        }
                    }

                    if (cookie.domain.size()) {
                        strutil::append(towrite, "; Domain=");
                        strutil::append(towrite, cookie.domain);
                    }
                    if (cookie.path.size()) {
                        strutil::append(towrite, "; Path=");
                        strutil::append(towrite, cookie.path);
                    }
                    if (cookie.secure) {
                        strutil::append(towrite, "; Secure");
                    }
                    if (cookie.httponly) {
                        strutil::append(towrite, "; HttpOnly");
                    }
                    if (cookie.samesite != SameSite::default_mode) {
                        strutil::append(towrite, "; SameSite=");
                        strutil::append(towrite, get_samesite(cookie.samesite));
                    }
                    return true;
                }
            };
