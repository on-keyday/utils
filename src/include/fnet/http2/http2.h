...
namespace utils {
    namespace fnet::http2 {
        ...
        struct fnet_class_export HTTP2Stream {
           private:
            friend struct HTTP2;
            void* opt = nullptr;
            int id_ = -1;
        ...
        struct fnet_class_export HTTP2 {
           private:
            void* opt = nullptr;
            int err = 0;
            constexpr HTTP2(void* p)
                : opt(p) {}
            friend fnet_dll_export(HTTP2) create_http2(bool server, h2set::PredefinedSettings settings);
