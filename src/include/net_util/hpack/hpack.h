...
namespace utils {
    namespace hpack {
        ...
        constexpr HpkErr encode_table_size_update(io::expand_writer<T>& w, std::uint32_t new_size, Table& table, OnModify&& on_modify_entry = OnModify{}) {
        ...
        constexpr HpkErr encode_field(io::expand_writer<T>& w, Table& table, auto&& key, auto&& value, std::uint32_t table_size_limit, bool adddymap = false, OnModify&& on_modify_entry = OnModify{}) {
            HpackError err = HpackError::none;
            size_t index = 0;
            // search table entry
            if (get_table_index(
                    [&](const auto& k, const auto& v) {
                        return helper::default_equal(k, key) && helper::default_equal(v, value);
                    },
                    index, table)) {
                ...
                if (get_table_index(
                        [&](const auto& k, const auto&) {
                            return helper::default_equal(k, key);
                        },
                        index, table)) {
                    if (adddymap) {
        ...
        template <class String, class Table, class Header, class T, class U, class OnModify = OnModifyTable>
        constexpr HpkErr decode_field(io::basic_reader<T, U>& r, Table& table, Header&& header, std::uint32_t& table_size_limit, std::uint32_t max_table_size, OnModify&& on_modify_entry = OnModify{}) {
            if (r.empty()) {
