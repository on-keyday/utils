...
namespace utils {
    namespace qpack {
        ...
        struct Context {
            ...
            io::expand_writer<String> enc_stream;
            io::expand_writer<String> dec_stream;
            std::uint64_t expires_delta = 0;

            QpackError read_encoder_stream(io::reader& r) {
                while (true) {
                    auto base = r.offset();
            ...
            QpackError read_header(StreamID id, io::reader& r, auto&& read) {
                fields::EncodedSectionPrefix enc_prefix;
