...
struct error {
   ...
   public:
    ...
    template <class T__>
    error(T__&& t) {
        static_assert(!std::is_same<std::decay_t<T__>,error>::value,"can't accept same type");
    ...
    constexpr error(error&& in) noexcept {
        iface=in.iface;
        in.iface=nullptr;
    }

    error& operator=(error&& in) noexcept {
...
struct Func {
   ...
   public:
    ...
    template <class T__>
    Func(T__&& t) {
        static_assert(!std::is_same<std::decay_t<T__>,Func>::value,"can't accept same type");
        if(!utils::helper::deref(t)){
            return;
        }
        iface = new implements__<std::decay_t<T__>>(std::forward<T__>(t));
    ...
    constexpr Func(Func&& in) noexcept {
        iface=in.iface;
        in.iface=nullptr;
    }

    Func& operator=(Func&& in) noexcept {
        if(this==std::addressof(in))return *this;
        delete iface;
        iface=in.iface;
        in.iface=nullptr;
