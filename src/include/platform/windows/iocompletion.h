...
struct CCBRegistered {

   private:
    struct vtable__t {
        bool (*call)(void* this__, void* ol, size_t com) = nullptr;
    };
    ...
    struct vtable__instance__ {
       private:
        ...
        static bool call(void* this__, void* ol, size_t com) {
            return static_cast<this_type*>(this__)->call(ol, com);
        }
   ...
   private:
    ...
    struct NOVTABLE__ interface__ {
        virtual bool call(void* ol, size_t com) = 0;
        virtual vtable__interface__ vtable__get__() const noexcept = 0;
        virtual interface__* move__(void* __storage_box) = 0;
    ...
    struct implements__ : interface__ {
        ...
        bool call(void* ol, size_t com) override {
            auto t_ptr_ = utils::helper::deref(this->t_holder_);
            if (!t_ptr_) {
                return bool{};
            }
   ...
   public:
    constexpr CCBRegistered(){}

    constexpr CCBRegistered(std::nullptr_t){}

    template <class T__>
    CCBRegistered(T__&& t) {
        static_assert(!std::is_same<std::decay_t<T__>,CCBRegistered>::value,"can't accept same type");
        if(!utils::helper::deref(t)){
            return;
        }
        new___(std::forward<T__>(t));
    ...
    CCBRegistered(CCBRegistered&& in) noexcept {
        // reference implementation: MSVC std::function
        if (in.is_local___()) {
            iface = in.iface->move__(__storage_box);
            in.delete___();
    ...
    CCBRegistered& operator=(CCBRegistered&& in) noexcept {
        if(this==std::addressof(in))return *this;
        delete___();
        // reference implementation: MSVC std::function
        if (in.is_local___()) {
    ...
    bool call(void* ol, size_t com) {
        return iface?iface->call(ol, com):bool{};
    }
    ...
    CCBRegistered(const CCBRegistered&) = delete;

    CCBRegistered& operator=(const CCBRegistered&) = delete;

    CCBRegistered(CCBRegistered&) = delete;

    CCBRegistered& operator=(CCBRegistered&) = delete;
