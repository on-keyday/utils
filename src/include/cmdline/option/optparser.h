...
struct Value {
   ...
   public:
    ...
    Value(Value& in) : Value(const_cast<const Value&>(in)) {}
...
struct OptParser {
   ...
   private:
    ...
    struct NOVTABLE__ interface__ {
        virtual bool is_parsable(const CmdParseState& ctx) = 0;
        virtual bool parse(Value& val, CmdParseState& ctx) = 0;
    ...
    struct implements__ : interface__ {
        ...
        bool is_parsable(const CmdParseState& ctx) override {
            auto t_ptr_ = utils::helper::deref(this->t_holder_);
            if (!t_ptr_) {
                return bool{};
            }
            return t_ptr_->is_parsable(ctx);
        }

        bool parse(Value& val, CmdParseState& ctx) override {
            auto t_ptr_ = utils::helper::deref(this->t_holder_);
            if (!t_ptr_) {
                return bool{};
            }
            return t_ptr_->parse(val, ctx);
        }
   ...
   public:
    ...
    bool is_parsable(const CmdParseState& ctx) {
        return iface?iface->is_parsable(ctx):bool{};
    }

    bool parse(Value& val, CmdParseState& ctx) {
        return iface?iface->parse(val, ctx):bool{};
    }
