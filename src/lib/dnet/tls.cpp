...
namespace utils {
    ...
    namespace dnet::tls {
        ...
        dnet_dll_implement(bool) is_open_ssl() {
            return lazy::crypto::ossl::sp::CRYPTO_set_mem_functions_.find();
        }

        dnet_dll_implement(bool) is_boring_ssl() {
        ...
        dnet_dll_implement(std::pair<TLS, error::Error>) create_tls_with_error(const TLSConfig& conf) {
            if (!conf.ctx) {
        ...
        dnet_dll_export(std::pair<TLS, error::Error>) create_quic_tls_with_error(const TLSConfig& conf, int (*cb)(void*, quic::crypto::MethodArgs), void* user) {
            if (!conf.ctx) {
                return {TLS{}, errConfigNotInitialized};
            }
            if (!cb) {
        ...
        constexpr error::Error errNotSupport = error::Error("not supported", error::ErrorCategory::dneterr);
        constexpr error::Error errLibJudge = error::Error("library type judgement failure. maybe other type library?", error::ErrorCategory::dneterr);
...
#define CHECK_QUIC_CONN(ctx, ...)                      \
    ...
    if (!ctx->quic_cb) {                               \
        ...
        error::Error TLS::setup_quic(int (*cb)(void*, quic::crypto::MethodArgs), void* user) {
            ...
            if (!quic::crypto::set_quic_method(ssl)) {
                return error::Error("libssl doesn't have QUIC extension", error::ErrorCategory::dneterr);
            }
            c->ssl = ssl;
            c->quic_cb = cb;
            c->user = user;
        ...
        dnet_dll_implement(void) set_libssl(lazy::dll_path path) {
            lazy::libssl.replace_path(path, false);
        }

        dnet_dll_implement(void) set_libcrypto(lazy::dll_path path) {
