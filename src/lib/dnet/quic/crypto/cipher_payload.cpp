...
namespace utils {
    namespace dnet {
        namespace quic::crypto {
            ...
            struct CipherMethodError {
                dnet::String method;

                void error(auto&& pb) {
                    helper::appends(pb, "unknown cipher method: ", method);
                }
            };

            dnet_dll_implement(error::Error) cipher_payload(const TLSCipher& cipher, CryptoPacketInfo info, ByteLen key, ByteLen iv_nonce, bool enc) {
                if (cipher == TLSCipher{}) {
                    // initial packet
                ...
                if (!meth) {
                    ...
                    return CipherMethodError{cipher.rfcname()};
                }
