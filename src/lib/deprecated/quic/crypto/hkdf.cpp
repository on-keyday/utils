    ...
    namespace quic {
        namespace crypto {
            ...
            bool make_initial_keys(InitialKeys& key,
                ...
                auto err = HKDF_Extract(initial, initial_len, client_conn_id, len);
                if (!err) {
                    return false;
                }
                if (mode == client) {
                    err = HKDF_Expand_label(key.initial, initial, initial_len, client_in);
                }
                else {
                    err = HKDF_Expand_label(key.initial, initial, initial_len, server_in);
                }
                err = err &&
